<x-site-layout title="Edit Match Article">
    ...
    <form action="{{route('admin.articles.update', $article)}}" method="post" class="w-2/3 border border-gray-300 p-4">
        ...
        <x-form-select name="stadia_id" value="{{$article->stadia_id}}" :options="\App\Models\Stadium::pluck('name', 'id')->toArray()" />
        ...
        <div class="w-full flex justify-end gap-x-8">
            ...
            <button type="submit" class="text-xs text-green-700 bg-green-300 hover:bg-green-200 px-4 py-2 rounded uppercase">Submit match article changes</button>
        </div>
    </form>
